...
public class ModuleSet : ICollection<Module> {
	...
	private long lastItemUsageMask {
		get {
			return ((long)1 << (ModuleData.currentModules.Length % 64)) - 1;
		}
	}
	...
	public ModuleSet(bool initializeFull = false) {
		this.data = new long[ModuleData.currentModules.Length / bitsPerItem + (ModuleData.currentModules.Length % bitsPerItem == 0 ? 0 : 1)];
	...
	public IEnumerator<Module> GetEnumerator() {
		...
		for (int i = 0; i < this.data.Length; i++) {
			...
			for (int j = 0; j < bitsPerItem; j++) {
				if ((value & ((long)1 << j)) != 0) {
					yield return ModuleData.currentModules[index];
				}
				index++;
				if (index >= ModuleData.currentModules.Length) {
					yield break;
				}
			}
		}
